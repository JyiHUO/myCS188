    ...
    def chooseAction(self, gameState):
        ...
        for belief in livingGhostPositionDistributions:
            ...
            for pos in belief.keys():
                if best_val < belief[pos]:
                    best_val = belief[pos]
                    best_pos = pos
            ghost_best_pos.append(best_pos)

        best_action = None
        best_val = 10**8
        for a in legal:
            next_pac_pos = Actions.getSuccessor(pacmanPosition, a)
            for ghost_pos in ghost_best_pos:
                dist = self.distancer.getDistance(next_pac_pos, ghost_pos)
                if best_val > dist:
                    best_val = dist
                    best_action = a
        return best_action
